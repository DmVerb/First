    ...
    protected StringBuilder compute() {
        ...
        try {
            ...
            Elements foundLinks = document.select("a[href~=((^https:\\/\\/[^\\/]*skillbox.*\\/[^\\?]+)|(^\\/.*\\/))]")
                    ...
                    .select("a[href~=(^((?!\\?).)*$)]")
                    .select("a[href~=(^((?!\\#).)*$)]");
            for (Element fooundlink : foundLinks){
                WebPage webPage = new WebPage(fooundlink.absUrl("href"));
                if (this.site.addPage(webPage)){
                    webPages.add(webPage);
                }
            }
        } catch (Exception e){
            System.out.println("Дочерние ссылки страницы " + this.startingUrl +" отсутствуют. Продолжаем поиск.");
        }
        if(webPages.size() > 0) {
